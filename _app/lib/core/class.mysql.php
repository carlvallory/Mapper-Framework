...
class Mysql {
    ...
    public function Execute($sql) {
        ...
        if (!empty($sql)):
			/*mysqli_query($this->Conn, "SET lc_time_names = 'es_ES';");*/
					...
					foreach($validations as $validation):
						...
						switch($validation):
							...
							case "number":
								...
								break;
							case "unique":
								list($prefix,$n) = explode("_", $fieldName);
								$sql = "SELECT {$fieldName} FROM " . $ob->tableName . " WHERE {$fieldName} = '{$field['value']}' AND {$prefix}_hidden = 0 AND " . $ob->primaryKey . " != {$id}";
								$res = $this->execute($sql);
