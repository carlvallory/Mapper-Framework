...
class Pagination extends Mysql {
    ...
    public function get($table, $limit = 20, $fields = NULL, $pageNumber = 1, $conditions = NULL) {
        ...
        if ($limit > 0):
            ...
            $navigationPrior = '<li><a href="'.str_replace("index","",substr($_SERVER['SCRIPT_NAME'],0,-4)).'/page/'.$priorNumber.$params.'"' . $this->clickcommand($priorNumber) . '>' . $strPrior . '</a></li>';
            $navigationNext  = '<li><a href="'.str_replace("index","",substr($_SERVER['SCRIPT_NAME'],0,-4)).'/page/'.$nextNumber.$params.'"' . $this->clickcommand($nextNumber) . '>' . $strNext . '</a></li>';
            ...
            if ($pages['lower'] > 2):
                $firstPage = '<li><a href="'.str_replace("index","",substr($_SERVER['SCRIPT_NAME'],0,-4)).'/page/1'.$params.'"' . $this->clickcommand(1) . '>1...</a></li>';
			else:
				$firstPage = NULL;
            endif;

            if (($pages['upper'] < $pages['pages'])):
                $lastPage = '<li><a href="'.str_replace("index","",substr($_SERVER['SCRIPT_NAME'],0,-4)).'/page/'.$pages['pages'].$params.'"' . $this->clickcommand($pages['pages']) . '>...' . $pages['pages'] . '</a></li>';
			else:
				$lastPage = NULL;
            endif;
            ...
            for($page = $pages['lower']; $page <= $pages['upper']; $page++):
                ...
                if($page == $this->pageNumber):
                    ...
                    $navigationNumbers .= '<li> <a href="'.str_replace("index","",substr($_SERVER['SCRIPT_NAME'],0,-4)).'/page/' . $page .$params. '"' . $this->clickcommand($page) . '>' . $page . '</a> </li>';
                endif;

            endfor;

            if($pages['pages'] > 1):
                $navigation = '<ul class="pagination">'.$navigationPrior .' '. $firstPage .' '. $navigationNumbers .' '. $lastPage .' '. $navigationNext.'</ul>';
			else:
				$navigation = NULL;
            endif;
